...
const ReportsList: React.FC<ReportsListProps> = ({ reports, title }) => {
  ...
  const filteredReports = reports.filter((report) => {
    ...
    const matchesDateFrom =
      !filters.dateFrom ||
      new Date(report.test_date) >= new Date(filters.dateFrom);
    const matchesDateTo =
      !filters.dateTo || new Date(report.test_date) <= new Date(filters.dateTo);
    const matchesRoad =
      !filters.road ||
      report.road_name?.toLowerCase().includes(filters.road.toLowerCase());
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-muted/30 rounded-lg">
          ...
          <div>
            ...
            <Input
              ...
              value={filters.road}
              onChange={(e) =>
                setFilters((prev) => ({ ...prev, road: e.target.value }))
              }
            />
          </div>
        </div>
                    ...
                    <div className="mt-3 p-3 bg-muted/30 rounded-lg">
                      ...
                      {(report.data_json || report.results) && (
                        <div className="text-xs space-y-1">
                          {report.data_json &&
                          typeof report.data_json === "object" ? (
                            Object.entries(report.data_json)
                              .slice(0, 2)
                              .map(([key, value]) => (
                            ...
                            <span>
                              Test data available - Click view for details
                            </span>
                          )}
                        </div>
                      )}
