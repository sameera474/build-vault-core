-- Migration: Rebuild profiles table - FINAL VERSION
-- Handles all role types including 'supervisor'
...
SELECT 
  ...
  email,
  company_id,
  COALESCE(tenant_role::text, role::text, 'technician') as old_role,
  is_super_admin,
  avatar_url,
  phone,
  department,
...
-- Success!
COMMENT ON TABLE profiles IS 'Profiles table rebuilt successfully. Role mapping: supervisor → project_manager';
...
BEGIN
  ...
  RAISE NOTICE 'Migration completed! Role distribution: %', role_stats;
END $$;
