-- Fix allocate_report_number to work with RLS policies
-- The function needs to query from a source that respects the user's access
...
DECLARE 
  ...
  v_lab text;
BEGIN
  -- Get project prefix and codes from my_projects view (respects RLS and user access)
  SELECT mp.project_prefix, mp.region_code, mp.lab_code
    INTO v_pref, v_reg, v_lab
  FROM public.my_projects mp 
  WHERE mp.id = _project AND mp.company_id = _company;
