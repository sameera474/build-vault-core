-- Migration: Rebuild profiles table with correct role system
...
SELECT 
  ...
  email,
  company_id,
  COALESCE(tenant_role, role, 'technician') as tenant_role,  -- Merge role columns
  is_super_admin,
  avatar_url,
  phone,
  department,
...
-- Step 2: Drop the old profiles table
DROP TABLE IF EXISTS profiles CASCADE;

-- Step 3: Create new profiles table with correct structure
CREATE TABLE profiles (
  ...
  -- Single role column - NO MORE role/tenant_role confusion!
...
-- Step 9: Clean up backup table (optional - comment out if you want to keep backup)
...
-- Success message
COMMENT ON TABLE profiles IS 'Rebuilt profiles table with single tenant_role column. Migration completed successfully.';
