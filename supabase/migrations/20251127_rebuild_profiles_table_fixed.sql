-- Migration: Rebuild profiles table with correct role system (FIXED)
-- This completely remakes the profiles table with proper role handling
-- Backs up existing data and restores it
-- FIX: Properly handles ENUM types by casting to TEXT

-- Step 1: Create backup of existing profiles data with proper type casting
...
SELECT 
  ...
  COALESCE(
    ...
    role::text,
    'technician'
  ) as tenant_role,  -- Cast ENUMs to TEXT first
  is_super_admin,
  avatar_url,
  phone,
  department,
...
-- Step 2: Drop the old profiles table and any ENUM types
...
-- Step 3: Create new profiles table with TEXT column (no ENUM!)
CREATE TABLE profiles (
  ...
  -- Single role column as TEXT with CHECK constraint - NO ENUM!
...
-- Admins can view and manage their company's profiles
...
-- Project managers can view their company's profiles
...
-- Step 9: Clean up backup table (optional - uncomment to delete backup)
...
-- Success message
COMMENT ON TABLE profiles IS 'Rebuilt profiles table with single tenant_role TEXT column (no ENUM). Migration completed successfully.';

-- Verification query (run separately to check results)
-- SELECT user_id, name, email, tenant_role, is_super_admin, company_id FROM profiles LIMIT 10;
